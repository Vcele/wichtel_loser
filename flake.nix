{
  ...
  outputs = { self, nixpkgs, rust-overlay, flake-utils }:
    let
      # 1. Define system-specific outputs (packages, apps, devShells)
    ...
    in
    # 2. Merge with system-agnostic outputs (nixosModules)
    systemOutputs // {
      nixosModules.default = { config, lib, pkgs, ... }:
        with lib;
        let
          cfg = config.services.wichtel-loser;
          # 3. Reference the package from self.packages using the system from the module arguments
          wichtelLoserPackage = self.packages.${pkgs.system}.default;
        in
        {
          ...
          config = mkIf cfg.enable {
            systemd.services.wichtel-loser = {
              ...
              serviceConfig = {
                ExecStart = "${wichtelLoserPackage}/bin/wichtel_loser";
                WorkingDirectory = "${wichtelLoserPackage}/share/wichtel_loser";
                Restart = "always";
                RestartSec = 5;
                DynamicUser = true;
              };
